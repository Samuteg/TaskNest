...
export const sendPasswordResetEmail = async ({
  ...
  if (!apiKey || !from) {
    console.info(
      `[auth] Password reset requested for ${to}. Configure RESEND_API_KEY and RESEND_FROM_EMAIL to send e-mails.`,
    );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Resend API error (${response.status}): ${responseText || "unknown error"}`,
      );
    }
