...
import { canUserAccessProjectWithRole, getAccessibleProjectIds, getProjectAccessMapForUser, } from "../lib/teamAccess.js";
// 1. CRIAR UM PROJETO
...
export const updateProject = async (req, res) => {
    try {
        ...
        const { allowed, project: existingProject } = await canUserAccessProjectWithRole(req.user, id, "admin");
        if (!existingProject) {
        ...
        const project = await Project.findByIdAndUpdate(id, { name: normalizedName }, { new: true });
        if (!project) {
            return res.status(404).json({ message: "Projeto não encontrado." });
        }
