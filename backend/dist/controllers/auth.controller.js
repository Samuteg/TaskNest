...
const getPasswordResetTokenTtlMs = () => {
    const ttlMinutes = Number.parseInt(ENV.PASSWORD_RESET_TOKEN_TTL_MINUTES || "", 10);
    const safeMinutes = Number.isFinite(ttlMinutes) && ttlMinutes > 0
        ? ttlMinutes
        : PASSWORD_RESET_DEFAULT_TTL_MINUTES;
    return safeMinutes * 60 * 1000;
};
const buildPasswordResetUrl = ({ email, token }) => {
    const frontendBaseUrl = ENV.FRONTEND_URL?.replace(/\/+$/, "") || "http://localhost:3000";
...
const forgotPasswordGenericMessage = "Se o e-mail estiver cadastrado, enviaremos instruções para redefinir a senha.";
...
export const uploadProfilePicture = async (req, res) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(req.user._id, { profilePic: uploadResult.secure_url }, { new: true }).select("-password");
        res.status(200).json({
