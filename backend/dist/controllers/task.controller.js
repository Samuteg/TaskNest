...
const parseDueDate = (rawDueDate) => {
    if (typeof rawDueDate === "undefined" ||
        rawDueDate === null ||
        rawDueDate === "") {
        return { value: null };
    }
    const normalizedInput = typeof rawDueDate === "string" ? rawDueDate.trim() : rawDueDate;
...
export const getTasks = async (req, res) => {
    try {
        ...
        const { allowed, project } = await canUserAccessProjectWithRole(req.user, projectId, "viewer");
        if (!project) {
...
export const createTask = async (req, res) => {
    try {
        ...
        const checklistInput = typeof req.body?.checklist === "undefined" ? [] : req.body.checklist;
        if (!title || !project) {
        ...
        const { allowed, project: existingProject } = await canUserAccessProjectWithRole(req.user, project, "editor");
        if (!existingProject) {
            return res.status(404).json({ message: "Projeto não encontrado." });
        }
        if (!allowed) {
        ...
        const nextOrder = typeof latestTask?.order === "number" ? latestTask.order + 1 : 0;
...
export const updateTask = async (req, res) => {
    try {
        ...
        const { allowed } = await canUserAccessProjectWithRole(req.user, task.project, "editor");
        if (!allowed) {
            return res.status(403).json({ message: "Sem permissão para este projeto." });
        }
        const updateData = {};
        ...
        if (Object.prototype.hasOwnProperty.call(updateData, "status") &&
            ...
            changedParts.push(`status para ${statusLabelByValue[updateData.status] || updateData.status}`);
        }
        ...
        if (Object.prototype.hasOwnProperty.call(updateData, "checklist") &&
